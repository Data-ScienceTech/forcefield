"""LangChain callback handler -- scan prompts and outputs through ForceField.
...
if _LANGCHAIN_AVAILABLE:
    ...
    class ForceFieldCallbackHandler(BaseCallbackHandler):
        """LangChain callback that scans prompts before they reach the LLM
        and moderates outputs after generation.

        Args:
            sensitivity: Detection sensitivity level.
            block_on_input: Raise ``PromptBlockedError`` if input is blocked.
            moderate_output: Run output moderation on LLM responses.
            on_block: Optional callable ``(scan_result) -> None`` for custom handling.
        """
            ...
            for prompt in prompts:
                ...
                if result.blocked:
                    logger.warning(
                        ...
                        result.risk_score, result.rules_triggered,
                    )
                    if self.on_block:
                        ...
                        if text:
                            ...
                            if not mod.passed:
                                logger.warning(
                                    ...
                                    mod.action.value, mod.categories,
                                )
            except Exception:
                pass
    ...
    class ForceFieldCallbackHandler:  # type: ignore[no-redef]
        """Placeholder -- install ``langchain-core`` to use this handler."""
        def __init__(self, *args: Any, **kwargs: Any):
            raise ImportError(
                "ForceFieldCallbackHandler requires langchain-core. "
